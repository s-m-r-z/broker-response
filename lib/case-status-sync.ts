...
import { deriveCaseStatus } from './case-tracker'
...
export async function syncCaseStatus(caseId: string): Promise<string | null> {
  ...
  await prisma.$transaction([
    ...
    prisma.caseActionLog.create({
      data: { caseId, type: 'STATUS_CHANGED', note: `${kase.status} → ${nextStatus}` },
    }),
  ])
  return nextStatus
}
  ...
  if (changed.length > 0) {
    await prisma.$transaction(
      changed.flatMap(({ kase, nextStatus }) => [
        ...
        prisma.caseActionLog.create({
          data: { caseId: kase.id, type: 'STATUS_CHANGED', note: `${kase.status} → ${nextStatus}` },
        }),
      ])
    )
  }
