...
export function ResponseList({
  ...
  return (
    <div className="flex h-full w-80 shrink-0 flex-col border-r border-zinc-800 bg-zinc-950">
      {/* Header */}
        ...
        {loading ? (
          ...
          responses.map((r) => (
            <div
              ...
              onClick={() => onSelect(r)}
              className={cn(
                'group flex cursor-pointer flex-col gap-1 border-b border-zinc-800/60 px-3 py-3 transition-colors hover:bg-zinc-900',
                selectedId === r.id && 'bg-zinc-900 border-l-2 border-l-blue-600'
              )}
            >
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate text-sm font-medium text-zinc-100">{r.brokerName}</p>
                    <span className="shrink-0 text-[10px] text-zinc-600">{formatRelativeTime(r.createdAt)}</span>
                  </div>
