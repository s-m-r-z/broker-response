...
function VerifiedBadge({ verified }: { verified: boolean }) {
  if (!verified) {
    return (
      <InfoTooltip content="Drafted by the mock AI provider — not yet reviewed by legal counsel. Do not cite to a broker until verified.">
        <span className="text-[11px] font-medium text-zinc-400 dark:text-zinc-500">AI-drafted · unverified</span>
      </InfoTooltip>
    )
  }
  return (
...
export function ClauseCard({ clause, onToggleVerified, onReviewChange }: ClauseCardProps) {
  ...
  return (
    <div data-testid={`clause-card-${clause.id}`} className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex items-start justify-between gap-3">
        ...
        <Button
          size="sm"
          variant={clause.verified ? 'outline' : 'secondary'}
          disabled={busy}
          onClick={handleVerifyToggle}
          data-testid={`clause-card-${clause.id}-verify-toggle`}
        >
